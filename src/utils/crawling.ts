...
const crawling = async (provider: number) => {
  ...
  if (provider === 1) {
    const brand = async () => {
      ...
      const brandDir = path.join(__dirname, '..', '..', 'public', 'brand', nowDate);

      const prodDir = path.join(__dirname, '..', '..', 'public', 'brand', nowDate, 'product');
      ...
      for (const [idx, el] of els.entries()) {
        const [brandNameEl, brandDescEl, brandThumbEl, prodThumbEls] = await Promise.all([
          el.$(brandTitleSel),
          el.$(brandDescSel),
        ...
        for (const prodThumbEl of prodThumbEls) {
          const prodThumb = await prodThumbEl.evaluate((el) => el.src!.trim())!;
        ...
        console.log(`%c Crawling in Progress... [${idx + 1}/${els.length}]`, progress);
      }
    ...
    const shopping = async (cateNum: number, maxPageNum: number, cate: string) => {
      type ProdTypes = {
        brand_name: string;
        product_name: string;
        off_rate: number;
      ...
      const prodDir = path.join(__dirname, '..', '..', 'public', 'product', nowDate);
      ...
      for (let i = 1; i <= maxPageNum; i++) {
        await page.goto(`https://kkst.kr/shopping/?cate_no=${cateNum}&page=${i}`);

        const baseSel = 'div.shopping-wr > div.ec-base-product ul.prdList > li.xans-record-';
        ...
        for (const el of els) {
          ...
          const [brandName, prodName, offRate, price, originPrice, prodThumb, soldOut] =
            await Promise.all([
              brandNameEl?.evaluate((el) =>
                el.textContent!.replace('9999아카이브', '9999 아카이브').trim(),
              )!,
              prodNameEl?.evaluate((el) => el.textContent!.trim())!,
              offRateEl?.evaluate((el) => Number(el.textContent?.replaceAll('%', '').trim())) ?? 0,
              priceEl?.evaluate((el) =>
                Number(el.textContent?.replaceAll('₩', '').replaceAll(',', '').trim()),
              ) ?? 0,
        ...
        console.log(`%c Crawling in Progress... [${i}/${maxPageNum}]`, progress);
      }
    ...
    for (const [idx, { cate_no, maxPageNum, cate }] of shoppingCase.entries()) {
      console.log(`%c Overall Progress [${idx + 1}/${shoppingCase.length}]`, progress);
